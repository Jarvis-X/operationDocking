...
if __name__ == "__main__":
    ...
    try:
        ...
        #     d.set_position(np.array([x[i], y[i], z[i]]))
        #     d.set_orientation(np.array([roll[i], pitch[i], yaw[i]]))
        ...
        while True:
            time_start = time.time()
            # d.set_position(np.array([np.cos(time_start/10), np.sin(time_start/12+0.5), 1.8+0.2*np.cos(time_start/9.0)]))
            # d.set_position(np.array([2*np.cos(time_start/2), 1.5*np.sin(time_start/2.5), 5+0.2*np.cos(time_start/3)]))
            # d.set_orientation(np.array([0.4*np.sin(time_start/4.5), 0.5*np.cos(time_start/5.0), np.pi*np.cos(time_start/8.0)]))
            # d.set_orientation(np.array([time_start/10.0, 0, 0]))
            # r1.control(pos, d.get_quaternion(), d.get_orientation(), d.get_velocity())
